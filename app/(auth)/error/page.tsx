...
const LoginErrorPage = () => {
  ...
  useEffect(() => {
    console.error("404 Error: `Error attempting to login`:", location.pathname)
  }, [location.pathname])
  ...
  return (
    <div className="min-h-screen w-full bg-login-gradient flex flex-col items-center justify-center p-4">
      <div className="login-card p-10 rounded-lg max-w-md w-full flex flex-col items-center text-center space-y-6 animate-fade-in">
        ...
        <Link href={"/login"}>
          <Button className="bg-white/10 backdrop-blur-md border border-white/10 hover:bg-white/20 text-white btn-social">
            Return to Login
          </Button>
        </Link>
      </div>

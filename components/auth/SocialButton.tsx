...
const SocialButton = ({ provider, onClick, className, style }: SocialButtonProps) => {
  ...
  return (
    <Button
      ...
      onClick={handleClick}
      className={cn(
        'btn-social w-full h-12 flex items-center justify-center gap-2 bg-background/10 backdrop-blur-md border border-white/10 hover:bg-white/20 transition-all',
        className
      )}
      style={style}
    >
      {isGoogleLoading && provider === 'google' ? (
        <FaSpinner className="animate-spin h-10 text-xl text-[#6330e4]" />
      ) : isGithubLoading && provider === 'github' ? (
        <FaSpinner className="animate-spin h-10 text-[#6330e4]" /> 
      ) : (
        <>
          {provider === 'google' && (
            <>
